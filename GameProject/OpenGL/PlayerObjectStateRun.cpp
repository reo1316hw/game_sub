...
{
	printf("Create : [PlayerObjectStateBase] PlayerObjectStateRun\n");
}

PlayerObjectStateRun::~PlayerObjectStateRun()
{
	printf("Remove : [PlayerObjectStateBase] PlayerObjectStateRun\n");
}
...
void PlayerObjectStateRun::Inipt(PlayerObject* _owner, const InputState& _keyState)
{
...
void PlayerObjectStateRun::Enter(PlayerObject* _owner, float _deltaTime)
{
	...
	meshcomp->PlayAnimation(_owner->GetAnim(PlayerState::PLAYER_STATE_RUN));
}

void PlayerObjectStateRun::MoveCalc(PlayerObject* _owner, float _deltaTime)
{
	/*if (buttonFlag == false)
	{
		charaSpeed -= 1.0f;
